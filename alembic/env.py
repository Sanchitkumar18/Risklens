...
Reads the database URL from application settings (not from ``alembic.ini``) so there
is a single source of truth and no secrets in version control. ``target_metadata`` is
bound to the ORM ``Base`` metadata so ``--autogenerate`` works.
"""
